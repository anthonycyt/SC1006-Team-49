...
def filterFood(request):
    ...
    if request.method == "GET":
        ...
        if min_price:
            # Check if either price or discountedPrice is greater than or equal to min_price
            foods = foods.filter(Q(price__gte=min_price) | Q(discountedPrice__gte=min_price))
        if max_price:
            # Check if either price or discountedPrice is less than or equal to max_price
            foods = foods.filter(Q(price__lte=max_price) | Q(discountedPrice__lte=max_price))
        if food_type:
            foods = foods.filter(type=food_type)
        if cuisine:
            foods = foods.filter(cuisine=cuisine)
        if discounted == "true":
            # Filter items that are either discounted or have a discounted price
            foods = foods.filter(isDiscounted=True, discountRate__gt=0) | foods.filter(discountedPrice__isnull=False)
